# Supabase Kurulum ve Veritabanı Şeması (Nalbur Stok, Kasa, Cari, Teklif & Çoklu Fiyat Listeleri)

Bu belgede, **SupaFastSale** sistemi için Supabase üzerinde yapılması gereken tüm veritabanı ayarları, tablolar, RLS politikaları, çoklu fiyat listeleri ve hazır nalbur demo verileri yer almaktadır.

---

## 1. Supabase Projesi Oluşturma & API Bilgileri

1. [Supabase](https://supabase.com) adresine gidip giriş yapın ve **New Project** butonuna tıklayın.
2. Proje ayarlarından **Project Settings > API** sekmesinden:
   - `Project URL`
   - `anon public key`
   bilgilerini alıp projedeki `js/supabase.js` dosyasına yapıştırın.
...
2. **Add User > Create User** butonuna tıklayarak dükkanınız için bir e-posta ve şifre oluşturun.
3. Kullanıcıların onay beklemeden hemen girebilmesi için: **Authentication > Providers > Email** sayfasından **Confirm email** seçeneğini pasif (disabled) yapıp kaydedin.

---

## 3. SQL Betiğini Çalıştırma (Tüm Tablolar, RLS ve İndeksler)

Supabase sol menüsünden **SQL Editor** sayfasına girin. **New Query** butonuna tıklayıp projenizdeki **`schema.sql`** dosyasının veya aşağıdaki SQL betiğinin tamamını yapıştırın ve **Run** butonuna basın:
...
-- Var olan politikaları güvenle temizle
...
-- products tablosuna görsel sütunu ekleme (var olan sistemler için)
alter table products add column if not exists image_url text;

-- product-images bucket'ı oluşturma (Public)
